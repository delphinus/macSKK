...
final class StateMachineTests: XCTestCase {
    ...
    @MainActor func testHandleNormalRomajiNQ() {
        ...
        stateMachine.inputMethodEvent.collect(10).sink { events in
            ...
            XCTAssertEqual(events[6], .modeChanged(.hankaku))
    ...
    @MainActor func testHandleNormalRomajiKanaRuleAzik() {
        ...
        XCTAssertTrue(stateMachine.handle(Action(keyBind: nil, event: generateNSEvent(character: ";", characterIgnoringModifiers: ";"))))
    ...
    @MainActor func testHandleNormalUnregisteredKeyEventWithModifiers() {
        ...
        XCTAssertFalse(stateMachine.handle(Action(keyBind: nil, event: generateNSEvent(character: "k", characterIgnoringModifiers: "k", modifierFlags: .control))))
        // Cmd-cもhandleせずfalseを返す
        XCTAssertFalse(stateMachine.handle(Action(keyBind: nil, event: generateNSEvent(character: "c", characterIgnoringModifiers: "c", modifierFlags: .command))))
    }
    ...
    @MainActor func testHandleNormalCtrlJ() {
        ...
        stateMachine.inputMethodEvent.collect(10).sink { events in
            ...
            XCTAssertEqual(events[2], .modeChanged(.direct))
            XCTAssertEqual(events[3], .modeChanged(.hiragana))
            XCTAssertEqual(events[4], .modeChanged(.eisu))
            XCTAssertEqual(events[5], .modeChanged(.hiragana))
            XCTAssertEqual(events[6], .modeChanged(.katakana))
            XCTAssertEqual(events[7], .modeChanged(.hiragana))
            XCTAssertEqual(events[8], .modeChanged(.hankaku))
            XCTAssertEqual(events[9], .modeChanged(.hiragana))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(hiraganaAction))
    ...
    @MainActor func testHandleNormalShiftQ() {
        ...
        stateMachine.inputMethodEvent.collect(9).sink { events in
            XCTAssertEqual(events[0], .modeChanged(.direct))
            XCTAssertEqual(events[1], .modeChanged(.hiragana))
            XCTAssertEqual(events[2], .modeChanged(.eisu))
    ...
    @MainActor func testHandleNormalAbbrevPrevMode() {
        ...
        stateMachine.inputMethodEvent.collect(12).sink { events in
            XCTAssertEqual(events[0], .modeChanged(.katakana))
            XCTAssertEqual(events[1], .modeChanged(.direct))
            XCTAssertEqual(events[2], .markedText(MarkedText([.markerCompose])))
    ...
    @MainActor func testHandleAbbrevUnegisteringPrevMode() {
        ...
        stateMachine.inputMethodEvent.collect(10).sink { events in
            ...
            XCTAssertEqual(events[1], .markedText(MarkedText([.markerCompose])))
            XCTAssertEqual(events[2], .markedText(MarkedText([.markerCompose, .plain("a")])))
            XCTAssertEqual(events[3], .markedText(MarkedText([.markerSelect, .emphasized("エイ")])))
            XCTAssertEqual(events[4], .markedText(MarkedText([.plain("a /エイ/ を削除します(yes/no)")])))
            XCTAssertEqual(events[5], .markedText(MarkedText([.plain("a /エイ/ を削除します(yes/no)"), .plain("y")])))
            XCTAssertEqual(events[6], .markedText(MarkedText([.plain("a /エイ/ を削除します(yes/no)"), .plain("ye")])))
            XCTAssertEqual(events[7], .markedText(MarkedText([.plain("a /エイ/ を削除します(yes/no)"), .plain("yes")])))
    ...
    @MainActor func testHandleNormalAbbrevCompletion() {
        ...
        stateMachine.inputMethodEvent.collect(6).sink { events in
            ...
            XCTAssertEqual(events[5], .modeChanged(.hiragana)) // abbrevモードでTab補完して確定したとき前のモードに戻す
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "/")))
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "a")))
        stateMachine.completion = .yomi(["apple"], 0)
    ...
    @MainActor func testHandleComposingNandQ() {
        ...
        stateMachine.yomiEvent.collect(2).sink { events in
            ...
            XCTAssertEqual(events[1], .other(""), "おnのように2文字目がローマ字の場合は空文字列が送信される")
            expectation.fulfill()
        }.store(in: &cancellables)
    ...
    @MainActor func testHandleComposingVandQ() {
        ...
        stateMachine.yomiEvent.collect(4).sink { events in
            ...
            XCTAssertEqual(events[1], .other("ゔ"))
            XCTAssertEqual(events[2], .other(""))
            XCTAssertEqual(events[3], .other("ゔ"))
            expectation.fulfill()
        }.store(in: &cancellables)
    ...
    @MainActor func testHandleComposingYomi() {
        ...
        stateMachine.yomiEvent.collect(5).sink { events in
            XCTAssertEqual(events[0], .other(""))
            XCTAssertEqual(events[1], .other("そ"), "カタカナモードでも読みにはひらがなが流れる")
            XCTAssertEqual(events[2], .other("そっ"), "tのように未確定のローマ字が入力中の場合はローマ字の前までが送信される")
            XCTAssertEqual(events[3], .other("そっと"), "「そっt」の状態ではローマ字を含むので送信しない")
            XCTAssertEqual(events[4], .other(""))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "q")))
    ...
    @MainActor func testHandleComposingYomiQ() {
        ...
        stateMachine.yomiEvent.collect(3).sink { events in
            XCTAssertEqual(events[0], .other(""))
            XCTAssertEqual(events[1], .other("ちょ"))
            XCTAssertEqual(events[2], .other(""), "カタカナで確定した")
            expectation.fulfill()
        }.store(in: &cancellables)
    ...
    @MainActor func testHandleComposingBackspace() {
        ...
        stateMachine.yomiEvent.collect(4).sink { events in
            XCTAssertEqual(events[0], .other(""))
            XCTAssertEqual(events[1], .other("しゅ"))
            XCTAssertEqual(events[2], .other("し"))
            XCTAssertEqual(events[3], .other(""))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: ";")))
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "s", withShift: true)))
    ...
    @MainActor func testHandleComposingAbbrevBackspace() {
        ...
        stateMachine.inputMethodEvent.collect(9).sink { events in
            ...
            XCTAssertEqual(events[7], .modeChanged(.katakana))
            XCTAssertEqual(events[8], .markedText(MarkedText([])))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "/")))
    ...
    @MainActor func testHandleComposingOkuriCancel() {
        ...
        stateMachine.inputMethodEvent.collect(3).sink { events in
            ...
            XCTAssertEqual(events[2], .markedText(MarkedText([.markerCompose, .plain("あう")])))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "a", withShift: true)))
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "u", withShift: true)))
    ...
    @MainActor func testHandleRegisteringOkuriCancel() {
        ...
        stateMachine.inputMethodEvent.collect(4).sink { events in
            ...
            XCTAssertEqual(events[1], .modeChanged(.hiragana))
            XCTAssertEqual(events[2], .markedText(MarkedText([.plain("[登録：い*う]")])))
            XCTAssertEqual(events[3], .markedText(MarkedText([.markerCompose, .plain("いう")])))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "i", withShift: true)))
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "u", withShift: true)))
    ...
    @MainActor func testHandleSelectingRememberCursor() {
        ...
        stateMachine.inputMethodEvent.collect(8).sink { events in
            ...
            XCTAssertEqual(events[7], .markedText(MarkedText([.markerCompose, .plain("える"), .cursor, .plain("う")])))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "u", withShift: true)))
        XCTAssertTrue(stateMachine.handle(leftKeyAction))
